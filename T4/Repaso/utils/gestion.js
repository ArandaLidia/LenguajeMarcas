...
boton.addEventListener("click", () => {
    ...
    if (inputNombre.value.length > 0) {
        ...
        guardarSaludo(inputNombre.value, selectopciones.value); // Pasa nombre y opción
...
function guardarSaludo(nombre, opcion) {
    // SE USA CUANDO NO HAY ANIMACIONES
    listaSaludo.innerHTML += `<li>${nombre} (${opcion})</li>`;

    // ESTE SE USA PARA QUE LAS ANIMACIONES NO SE DUPLIQUEN. 
    /* let nodo = document.createElement("li");
    nodo.textContent = `${nombre} (${opcion})`; //añado texto al nodo lista.
    nodo.className = "nombre-clase"; //añado la clase al nodo li
